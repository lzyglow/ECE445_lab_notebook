...
Below is the final requirement and verification from the final report:
.
Theft detection system
Sensing Subsystem: We test the voltage ESP32 used to supply the wire is 3.26V using a multimeter as seen in the voltage supply from esp32s3, so the voltage across the wire is 3.26V. This voltage is much lower than 50V, ensuring human safety when touching. We also test the voltage supplied to the ESP32 is 3.26V, which is within the 3.0V±0.3V operational voltage range, as seen in the "voltage supplied to esp32S3 after regulator". 

Power Subsystem: For the detection system, we tested the battery voltage drop per hour is 0.57V as seen in "voltage that battery supply over time". Since the drop out voltage is 2V for the regulator that steps down the voltage to 3.3V and a 9V battery is used to power the detection system, the battery life is expected to be (9V-(3.3+2)V)/0.57V = around 6 hours , which is much higher than our expectation of 3 hours. The battery is 9V, and since the voltage supplied to the ESP32 in the detection system is 3,26 as seen in "voltage supplied to esp32S3 after regulator", the regulator successfully steps down the voltage to operation voltage and supplies the microcontroller.

Control subsystem: We use an online app and detect  91dB at around 5 feet when the alarm is triggered by the control subsystem, as shown in "db_test.png". During the testing, the alarm is recorded to trigger around 1.0 second after wire cut. The ESP32 is supplying a voltage of 3.26V to the wire in the sensing subsystem shown in the "voltage_supply_to_wire.jpg". 
Wireless camera system

Power system: We use a 9V battery to power the system. The operational voltage of the ESP32-CAM is around 5V. We use a LED light to indicate when voltage drops below 4.26V when a button is pressed, as shown in led.png. As shown in voltage_supply_to_esp32cam.jpg.png.jpg, the voltage supplied to the ESP32-CAM is 5.19V, which is around 5V. We failed to verify that the battery can last for around 3 hours however. Since the operational voltage of ESP32-CAM is 5V, it consumes more power than expected, and the battery is tested to last for around 2 hours. This unsatisfied requirement is described more detailedly in the conclusion section. We can confirm that the battery is expected to last for 2 hours. 

Camera Control Subsystem: See the graph below, we use a timer together with code to detect the time ESP32-CAM needs to receive a bluetooth signal from the detection system. The signal is fast in transfer, and about 1 second (avg 1.84) is needed for the ESP32-CAM to indicate the signal is received. The tolerance was changed to ± 1 second to account for user error, as we manually timed the bluetooth signal response for testing. The camera-consent-signal is observed to be sent instantly after the bluetooth signal is received (observed through serial monitor display), so the timing data is the same as the previous requirement’s verification. These requirements verify that both our code and the ESP32-CAM are fully functional. The timing between camera capture and cell phone notification is measured manually by observing camera  flash and Telegram app notification. This timing averages to 3.52 seconds, meeting timing requirements. Despite the requirements being restrictive, the numbers are overall practical for application. 
![time_of_camera_control_module](time_of_camera_control_module.png)
